...
dbname_old = 'ou150upgref'
dbname_new = 'ou160upgref'
modules = None  # all
create_old = True
create_new = True
...
def prepare_server(db_admin_pw, host, port, dbname, user_pw):
    ...
    install_single_module(client, 'upgrade_analysis')
    if not modules:
        print("Installing all modules")
        wizard = client.env['upgrade.install.wizard'].browse(
            client.env['upgrade.install.wizard'].create({}))
...
wizard_id = client_new.env['upgrade.analysis'].create({
    'config_id': get_or_create_config(client_new),
    'upgrade_path': '/home/bruno/buildout/14.0/parts/openupgrade/openupgrade_scripts/scripts',
})
client_new.env['upgrade.analysis'].analyze([wizard_id])
```
